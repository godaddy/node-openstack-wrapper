...
// Server methods
// ----------------
Nova.prototype.listServers = function(queryParams,cb)
{
  var self = this;
  var request_options = this.getRequestOptions('/servers/detail?'+ queryParams, true);
...
Nova.prototype.createMultipleServer = function(data, cb)
{
  var self = this;
  var request_options = this.getRequestOptions('/servers', data);
  request_options.metricPath = 'remote-calls.nova.servers.create-multiple';
  request_options.validateStatus = true;  
  ...
  this.request.post(request_options, function(error, response, body){
    ...
    cb(null, self.mangleObject('Server', body));
  });
};
