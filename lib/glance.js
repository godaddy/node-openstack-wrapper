...
//NOTE: I'm not sure the ?member_status=all is a standard openstack thing..
Glance.prototype.listImages = function(cb)
{
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    if(osutils.isError(error, response) || !body.images || !body.links)
    {
    ...
    {
      images_array[n] = this.mangleObject('Image', body.images[n]);
    }
    
    //tack these on for easy consupmtion and in case we ever need pagination
    images_array.self = body.links.self;
    images_array.previous = body.links.previous;
    images_array.next = body.links.next;
...
Glance.prototype.getImage = function(id, cb)
{
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    cb(null, this.mangleObject('Image', body));
  });
}
...
Glance.prototype.updateImage = function(id, data, cb)
{
  ...
  this.request.patch(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.id)
    {
      cb(osutils.getError('glance.updateImage', error, response, body)) ;
      return;
    }
...
Glance.prototype.removeImage = function(id, cb)
{
  ...
  this.request.del(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.id)
    {
      cb(osutils.getError('glance.removeImage', error, response, body));
      return;
    }
