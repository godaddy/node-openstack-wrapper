...
function _getRequestError(error, response, body, request_options, response_time)
{
  ...
  else if(request_options && request_options.requireBodyObject && getNestedPropValue(body, request_options.requireBodyObject, 'nope-nope-nope') == 'nope-nope-nope')
  {
  ...
  else if(body)
  {
    for(key in body)
    {
      if(body.hasOwnProperty(key) && body[key].hasOwnProperty('code'))
      {
  ...
  else if(body)
  {
    for(key in body)
    {
      if(body.hasOwnProperty(key) && body[key].hasOwnProperty('detail'))
      {
...
function _actualRequest(options, callback){
  ...
  Request(options, function(error, response, body){
    ...
    else if(options.requireValidStatus && (status_code <= 199 || status_code >= 300))
    {
