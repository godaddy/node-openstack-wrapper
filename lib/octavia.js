...
Octavia.prototype.createLBListener = function(loadbalancer_id, protocol, data, cb){
  var optional_keys = ['description', 'protocol_port', 'default_tls_container_ref', 'sni_container_refs', 'admin_state_up', 'name', 'connection_limit'];
...
Octavia.prototype.updateLBListener = function(listener_id, data, cb){
  ...
  Async.until(
    ...
    function(cb){
      count++;
      self.request.put(request_options, function(error, response, body){
...
Octavia.prototype.removeLBListener = function(listener_id, cb){
  ...
  Async.until(
    ...
    function(cb){
      count++;
      self.request.del(request_options, function(error, response, body){
        if(error && error.detail.remoteStatusCode == 409 && count <= self.retries)
        {
          //wait a second then call back to trigger another try and ignore this error
          setTimeout(cb, self.retry_delay);
  ...
  Async.until(
    ...
    function(cb){
      ...
      self.request.post(request_options, function(error, response, body){
        if(error)
        {
          console.log('failed to create pool - status code is: ', error.detail.remoteStatusCode);
          console.log('count is: ' + count + ' and retries are: ' + self.retries);
        }
        if(error && error.detail.remoteStatusCode == 409 && count <= self.retries)
        {
          //wait a second then call back to trigger another try and ignore this error
...
Octavia.prototype.createLBPoolMember = function(pool_id, address, protocol_port, data, cb){
  ...
  var count = 0; 
  var done = false;
  var optional_keys = ['name', 'monitor_address', 'admin_state_up', 'weight', 'subnet_id'];
