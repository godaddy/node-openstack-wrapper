...
Neutron.prototype.listFloatingIps = function(options, cb){
  ...
  var args = osutils.getFuncArgsWithCallback.apply(null, arguments);
  request_options.logPath = 'api-calls.neutron.floatingip-list';

  //if no callback throw error
  if(!args.callback)
  {
    custom_error = new Error('Invalid arguments to listFloatingIps, missing callback');
    custom_error.status= 422;
    Error.captureStackTrace(custom_error);
    cb(custom_error, null);
    return;
  }
  //else continue
...
Neutron.prototype.listPorts = function(options, cb){
  ...
  if(!args.callback)
  {
    custom_error = new Error('Invalid arguments to listPorts, missing callback');
    custom_error.status= 422;
    Error.captureStackTrace(custom_error);
    cb(custom_error, null);
    return;
