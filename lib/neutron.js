...
Neutron.prototype.listFloatingIps = function(options, cb){
  ...
  // check to see if function call was overloaded with options object
  if(typeof options == 'function')
  {
    ...
    {
      // invalid input parameters, throw an error
      // TODO: put real error in here
      cb('ERROR!', null);
    }
  }
  else 
  {
    // currently, options are just filters, iterate over options.filters keys and set them on query string
    filters = options.filters || filters;
    
    Object.keys(filters).forEach(function(filter_name){
      qs[filter_name] = filters[filter_name];
    });
...
//takes optional options object that would contain any filters to apply to request
// i.e.
//{
//  filters: 
//  {
...
Neutron.prototype.listPorts = function(options, cb){
  ...
  {
    // currently, options are just for filters, iterate over options.filters keys and set them on query string
    filters = options.filters || filters;
  ...
  console.log('listPorts request options  = ' + JSON.stringify(request_options));
  this.request.get(request_options, function(error, response, body){
    console.log('listPorts response = ' + JSON.stringify(response));
    console.log('listPorts error = ' + JSON.stringify(error));
    ...
    console.log('ports array = ' + JSON.stringify(ports_array));
