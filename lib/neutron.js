...
Neutron.prototype.listSubnets = function(cb){
  ...
  this.request.get(request_options, function(error, response, body){
    if(osutils.isError(error, response))
    {
      cb(osutils.getError('Neutron.listSubnets', error, response, body));
      return;
    }
...
Neutron.prototype.getSubnet = function(subnet_id, cb){
  ...
  this.request.get(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.subnet)
    {
      cb(osutils.getError('Neutron.getSubnet', error, response, body));
      return;
    }
    //else
...
Neutron.prototype.listListeners = function(cb){
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    {
      //leave an empty array as the result but go ahead and log just in case this is unintended.. er somethin
      console.error('No Listeners found for given project - returning blank array');
    }
...
Neutron.prototype.getListener = function(listener_id, cb){
  var self = this;
...
Neutron.prototype.listLBPools = function(cb){
  ...
  request_options.logPath = 'api-calls.neutron.lbaas-lbpool-list';
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    if(osutils.isError(error, response))
    {
      cb(osutils.getError('Neutron.listLBPools', error, response, body));
      return;
    }
    ...
    {
      //leave an empty array as the result but go ahead and log just in case this is unintended.. er somethin
      console.error('No LBPools found for given project - returning blank array');
    }
...
Neutron.prototype.getLBPool = function(pool_id, cb){
  ...
  this.request.get(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.pool)
    {
      cb(osutils.getError('Neutron.getLBPool', error, response, body));
      return;
    }
...
// Calls back cb(error, rule) with a newly created resource from the given params
Neutron.prototype.createLBPool = function(tenant_id, protocol, lb_algoritm, listener_id, data, cb){
  var self = this;
  ...
  post_data.pool.lb_algoritm = lb_algoritm;
  ...
  this.request.post(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.pool)
    {
      cb(osutils.getError('Neutron.createLBPool', error, response, body));
      return;
    }
...
Neutron.prototype.updateLBPool = function(pool_id, data, cb){
  var self = this;
  var optional_keys = ['name', 'description', 'admin_state_up', 'lb_algoritm', 'session_persistence'];
  var put_data = {pool: {}};
  var key = '';
  var n = 0;
  var request_options = {};
  ...
  this.request.put(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.pool)
    {
      cb(osutils.getError('Neutron.updateLBPool', error, response, body));
      return;
    }
...
Neutron.prototype.removeLBPool = function(pool_id, cb){
  var request_options = this.getRequestOptions('/lbaas/pools/' + pool_id, true);
  request_options.logPath = 'api-calls.neutron.lbaas-pools-remove';

  this.request.del(request_options, function(error, response, body){
    if(osutils.isError(error, response))
    {
      cb(osutils.getError('Neutron.removeLBPool', error, response, body));
      return;
    }
...
Neutron.prototype.listMembers = function(pool_id, cb){
  var self = this;
  var member_array = [];
  var n = 0;
  var request_options = this.getRequestOptions('/lbaas/pools/' + pool_id + '/members', true);
  request_options.logPath = 'api-calls.neutron.lbaas-pools-member-list';

  //console.log('Request Options:', request_options);
  this.request.get(request_options, function(error, response, body){
    if(osutils.isError(error, response))
    {
      cb(osutils.getError('Neutron.listMembers', error, response, body));
      return;
    }
    ...
    if(body.members && body.members.length)
    {
      ...
      {
        member_array[n] = self.mangleObject('Member', body.members[n]);
      }
    }
    else
    {
      //leave an empty array as the result but go ahead and log just in case this is unintended.. er somethin
      console.error('No Members found for given project - returning blank array');
    }
...
Neutron.prototype.getMember = function(pool_id, member_id, cb){
  var self = this;
  var request_options = this.getRequestOptions('/lbaas/pools/' + pool_id + '/members/' + member_id, true);
  request_options.logPath = 'api-calls.neutron.lbaas-pools-member-get';

  //console.log('Request Options:', request_options);
  this.request.get(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.member)
    {
      cb(osutils.getError('Neutron.getMember', error, response, body));
      return;
    }

    cb(null, self.mangleObject('Member', body.member));
  });
};


// Calls back cb(error, rule) with a newly created resource from the given params
Neutron.prototype.createMember = function(pool_id, tenant_id, address, protocol_port, data, cb){
  var self = this;
  var optional_keys = ['admin_state_up', 'weight'];
  ...
  request_options.logPath = 'api-calls.neutron.lbaas-pools-member-create';

  this.request.post(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.member)
    {
      cb(osutils.getError('Neutron.createMember', error, response, body));
      return;
    }
...
Neutron.prototype.updateMember = function(pool_id, member_id, data, cb){
  var self = this;
  ...
  this.request.put(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.member)
    {
      cb(osutils.getError('Neutron.updateMember', error, response, body));
      return;
    }
...
//calls back with (error) after attempting to remove the given resource
Neutron.prototype.removeMember = function(pool_id, member_id, cb){
  var request_options = this.getRequestOptions('/lbaas/pools/' + pool_id +'/members' + member_id, true);
  request_options.logPath = 'api-calls.neutron.lbaas-pools-member-remove';

  this.request.del(request_options, function(error, response, body){
    if(osutils.isError(error, response))
    {
      cb(osutils.getError('Neutron.removeMember', error, response, body));
      return;
    }
...
Neutron.prototype.listHealthMonitors = function(cb){
  var self = this;
  var request_options = this.getRequestOptions('/lbaas/healthmonitors', true);
  request_options.logPath = 'api-calls.neutron.lbaas-healthmonitors-list';
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    if(osutils.isError(error, response))
    {
      cb(osutils.getError('Neutron.listHealthMonitors', error, response, body));
      return;
    }
    ...
    if(body.healthmonitors && body.healthmonitors.length)
    {
      ...
      {
        healthmonitor_array[n] = self.mangleObject('HealthMonitor', body.healthmonitors[n]);
      }
    }
    else
    {
      //leave an empty array as the result but go ahead and log just in case this is unintended.. er somethin
      console.error('No health monitors found for given project - returning blank array');
    }
...
Neutron.prototype.getHealthMonitor = function(health_monitor_id, cb){
  var self = this;
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    if(osutils.isError(error, response) || !body.healthmonitor)
    {
      cb(osutils.getError('Neutron.getHealthMonitor', error, response, body));
      return;
    }

    cb(null, self.mangleObject('HealthMonitor', body.healthmonitor));
  });
};


// Calls back cb(error, rule) with a newly created resource from the given params
Neutron.prototype.createHealthMonitor = function(tenant_id, type, delay, timeout, max_retries, pool_id, data, cb){
  var self = this;
  ...
  this.request.post(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.healthmonitor)
    {
      cb(osutils.getError('Neutron.createHealthMonitor', error, response, body));
      return;
    }
...
Neutron.prototype.updateHealthMonitor = function(health_monitor_id, data, cb){
  var self = this;
  ...
  request_options = this.getRequestOptions('/lbaas/healthmonitor/' + health_monitor_id, put_data);
  request_options.logPath = 'api-calls.neutron.lbaas-healthmonitor-update';

  this.request.put(request_options, function(error, response, body){
    if(osutils.isError(error, response) || !body.healthmonitor)
    {
      cb(osutils.getError('Neutron.updateHealthMonitor', error, response, body));
      return;
    }
...
//calls back with (error) after attempting to remove the given resource
Neutron.prototype.removeHealthMonitor = function(health_monitor_id, cb){
  var request_options = this.getRequestOptions('/lbaas/healthmonitor/' + health_monitor_id, true);
  request_options.logPath = 'api-calls.neutron.lbaas-healthmonitor-remove';

  this.request.del(request_options, function(error, response, body){
    if(osutils.isError(error, response))
    {
      cb(osutils.getError('Neutron.removeHealthMonitor', error, response, body));
      return;
    }
...
Neutron.prototype.getStats = function(lb_id, cb){
  var self = this;
  var request_options = this.getRequestOptions('/lbaas/loadbalancers/' + lb_id + '/stats', true);
  request_options.logPath = 'api-calls.neutron.lbaas-loadbalancers-stat-get';

  //console.log('Request Options:', request_options);
  this.request.get(request_options, function(error, response, body){
    //console.log('stats body: ', body);
    if(osutils.isError(error, response) || !body.stats)
    {
      cb(osutils.getError('Neutron.getStats', error, response, body));
      return;
    }
    //else
